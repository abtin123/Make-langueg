...
Only missing keys or values still equal to the Persian source are translated.
The source VALUE is sent to the translator; key names are never translated.
Placeholders are protected before translation and verified afterwards.
"""
...
def google_batch(texts, target):
    ...
    result=[]
    for text in texts:
        result.extend(google_batch([text],target))
    return result
...
def main():
    ...
    if not fa or set(fa)!=set(en):
        sys.exit(f'fa/en source mismatch: fa={len(fa)} en={len(en)}')
    for code in LANGS:
        ...
        for k,v in fa.items():
            ...
            if PH.findall(v) and set(PH.findall(data[k])) != set(PH.findall(v)):
                sys.exit(f'{code}: placeholder mismatch in {k}')
    ...
    for code in LANGS:
        ...
        for k in fa:
            if PH.findall(fa[k]) and set(PH.findall(d[k]))!=set(PH.findall(fa[k])):
                sys.exit(f'{code}: placeholder parity failed: {k}')
