...
assert "lang_{code}.abl" in packer and "sha256" in packer and "download_url" in packer
assert "--fill-missing" in packer and "fallback_key_count" in packer
assert "complete legacy" in packer
assert "gh release upload" in workflow and "--clobber" in workflow
assert "--fill-missing" in workflow and "--skip-incomplete" not in workflow
assert "TRANSLATION_API" not in workflow
assert "lang_$code.abl" in catalog
