...
from generate_full_locales import LANGUAGES
...
def build(*, source_path: Path, locales_dir: Path, out_dir: Path, download_base: str, include_builtins: bool, codes: list[str] | None) -> None:
    ...
    for code, (english_name, native_name, flag, direction) in LANGUAGES.items():
        ...
        if set(values) != expected_keys:
            raise ValueError(f"{code} is incomplete; missing={len(expected_keys - set(values))}, extra={len(set(values) - expected_keys)}")
        raw = json.dumps(values, ensure_ascii=False, sort_keys=True, separators=(",", ":")).encode("utf-8")
        packed = gzip.compress(raw, compresslevel=9, mtime=0)
        file_name = f"lang_{code}.lpk"
...
def main() -> None:
    ...
    build(source_path=args.source, locales_dir=args.locales_dir, out_dir=args.out, download_base=args.download_base, include_builtins=args.include_builtins, codes=args.codes)
